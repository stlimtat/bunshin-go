...
func TestFragment_Validate_OK(t *testing.T) {
	f := &prompt.Fragment{ID: "f1", Variables: []string{"name", "domain"}}
...
func TestFragment_Validate_MissingVar(t *testing.T) {
	f := &prompt.Fragment{ID: "f1", Variables: []string{"name"}}
...
func TestMemoryBackend_PutAndGet(t *testing.T) {
	b := prompt.NewMemoryBackend()
	f := &prompt.Fragment{ID: "greet", Content: "Hello {{.name}}"}
	b.Put(f)
	got, err := b.Get(context.Background(), "greet")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got.ID != "greet" {
		t.Fatalf("want greet, got %q", got.ID)
	}
}

func TestMemoryBackend_List_NoFilter(t *testing.T) {
	b := newBackendWith(
		&prompt.Fragment{ID: "a", Tags: []string{"system"}},
		&prompt.Fragment{ID: "b", Tags: []string{"user"}},
	)
	frags, _ := b.List(context.Background())
	if len(frags) != 2 {
...
func TestMemoryBackend_List_TagFilter(t *testing.T) {
	b := newBackendWith(
		&prompt.Fragment{ID: "a", Tags: []string{"system", "persona"}},
		&prompt.Fragment{ID: "b", Tags: []string{"user"}},
		&prompt.Fragment{ID: "c", Tags: []string{"system"}},
	)
	frags, _ := b.List(context.Background(), "system")
	if len(frags) != 2 {
...
func TestMemoryBackend_GetVersion(t *testing.T) {
	b := prompt.NewMemoryBackend()
	b.Put(&prompt.Fragment{ID: "f", Version: "v1", Content: "old"})
	b.Put(&prompt.Fragment{ID: "f", Version: "v2", Content: "new"})

	v1, err := b.GetVersion(context.Background(), "f", "v1")
	if err != nil || v1.Content != "old" {
		t.Fatalf("want old, got %q %v", v1.Content, err)
	}
	v2, err := b.GetVersion(context.Background(), "f", "v2")
...
func TestMemoryBackend_Watch_ReceivesUpdate(t *testing.T) {
	...
	ch, err := b.Watch(ctx, "myf")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	b.Put(&prompt.Fragment{ID: "myf", Content: "v1"})
...
func TestPromptComposer_Render_Simple(t *testing.T) {
	...
	c := prompt.NewPromptComposer(b)
	tmpl := prompt.PromptTemplate{
		Fragments: []prompt.FragmentRef{{ID: "greet"}},
	}
...
func TestPromptComposer_Render_MultiFragment(t *testing.T) {
	b := newBackendWith(
		&prompt.Fragment{ID: "a", Content: "Part A"},
		&prompt.Fragment{ID: "b", Content: "Part B"},
	)
	c := prompt.NewPromptComposer(b)
	tmpl := prompt.PromptTemplate{
		Fragments: []prompt.FragmentRef{{ID: "a"}, {ID: "b"}},
		Separator: " | ",
	}
...
func TestPromptComposer_Render_Override(t *testing.T) {
	...
	tmpl := prompt.PromptTemplate{
		Fragments: []prompt.FragmentRef{
			{ID: "persona", Overrides: map[string]any{"role": "legal"}},
		},
	}
...
func TestPromptComposer_Render_MissingFragment(t *testing.T) {
	...
	c := prompt.NewPromptComposer(b)
	tmpl := prompt.PromptTemplate{
		Fragments: []prompt.FragmentRef{{ID: "nonexistent"}},
	}
...
func TestPromptComposer_Render_MissingVariable(t *testing.T) {
	...
	c := prompt.NewPromptComposer(b)
	tmpl := prompt.PromptTemplate{
		Fragments: []prompt.FragmentRef{{ID: "needs-var"}},
	}
