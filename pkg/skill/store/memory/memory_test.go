...
import (
	...
	"github.com/stlimtat/bunshin-go/pkg/skill/store/memory"
)
...
func TestStore_Resurrect_AfterDelete(t *testing.T) {
	s := memory.New()
	spec := makeSpec(t)
	ver, _ := s.Create(context.Background(), "t1", spec)
	s.Activate(context.Background(), "t1", "test-skill", ver) //nolint:errcheck
	s.Delete(context.Background(), "t1", "test-skill")        //nolint:errcheck
	// Resurrect by creating again.
	ver2, _ := s.Create(context.Background(), "t1", spec)
	if ver != ver2 {
		t.Errorf("same content should yield same version: %q vs %q", ver, ver2)
	}
	// Should be in draft state after resurrection.
	_, err := s.Get(context.Background(), "t1", "test-skill")
	if !errors.Is(err, skill.ErrNotFound) {
		t.Errorf("resurrected skill should be draft, not active")
	}
}
